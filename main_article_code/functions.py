...
def initialize_P(n_x, n_h, n_y):
    """
    Argument:
    n_x -- number of input in the training   m ?? 
    n_h -- number units in the hidden layer
    n_y -- number of output, which will be the number of dependent variable in the system.   n
...
def feed_forward(X, P1, P2, a1, a2, a, right_side):
       ...
       # define the trial solutions 
        def Yt1(X, P1):
            ...
            A2 = Z2                                  
            return a1 + (X-a)*(A2)[0]             # the first component - y1
        def Yt2(X, P2):
            W21 = P2["W21"]
            b21 = P2["b21"]
            W22 = P2["W22"]
            ...
            A2 = Z2                                    
            return a2 + (X-a)*(A2)[1]              # the second component - y2
        ...
        def loss(P1,P2):
            ...
            cost = 0.5*np.sum(cost1**2+cost2**2) # total cost - sum of two lost functions
            return cost
...
def ode_nn_model1(X, h,  a1, a2, a, right_side,tol, ITs, iter = 0, print_cost = False):
    ...
    """
    n_x = X.shape[0] # 1
    n_y =  2 # two components in our example
